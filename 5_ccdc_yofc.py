...
"""
Author: Dan Zelenak
Last Updated: 5/12/2017, updated 10/3/2017
Description:  Generate a raster that shows spatially the first year in which a change occurred based on the input
CCDC annual DOY change layers.  The output raster is unsigned 16-bit integer; each pixel contain 4 digits representing
the year in which the first change in the time-series occurred at that location.  0-value pixels signify that no change
occurred in the time series at that location.  Optionally, arguments can be passed for a beginning and/or end year to
generate the first year of change for a specific segment of the time series.  A color map is automatically applied to
the output raster.
"""
...
def all_calc_numpy(in_dir, out_dir, year1=None, year2=None):
    ...
    for file, year in zip(r_list_, y_list_):
