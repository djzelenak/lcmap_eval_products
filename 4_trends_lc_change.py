	...
	try:
		...
		for t in range(len(TrendsList)-1, -1, -1): # go backwards through the list to avoid index errors
			...
			if np.all(testbanddata == 0):
				...
				print "\nNo Trends data for this tile in year %s\n" %(TrendsYearList[t] ) # for testing
		...
		if FromY == None and ToY == None:
			...
			if not FromY in TrendsYearList:
				print "The -frm 'from' year  %s entered does not contain data for tile %s" %(FromY, ARD)
				print "The following years are valid:"
				pprint.pprint (TrendsYearList)
				sys.exit(1)
			if not ToY in TrendsYearList:
				print "The -to year %s entered does not contain data for tile %s" %(ToY, ARD)
				print "The following years are valid:"
				pprint.pprint (TrendsYearList)
				sys.exit(1)
			
			inputFile1 = "%s/Trendsblock_%s.tif" %(inDir, FromY)
			inputFile2 = "%s/Trendsblock_%s.tif" %(inDir, ToY)
...
def usage():
	print ''
	print('Usage:python 4_trends_lc_change.py\n\n \
...
def examps():
	print ''
	print('Usage: python 4_trends_lc_change.py -i /some/directory \
